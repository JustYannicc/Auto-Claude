...
import type { IPCResult, WorktreeStatus, WorktreeDiff, WorktreeDiffFile, WorktreeMergeResult, WorktreeDiscardResult, WorktreeListResult, WorktreeListItem, SupportedIDE, SupportedTerminal, CreatePullRequestResult } from '../../../shared/types';
...
export function registerWorktreeHandlers(
   ...
   * Auto-pushes the branch to remote if not already pushed
   */
  ipcMain.handle(
    IPC_CHANNELS.TASK_CREATE_PR,
    async (_, taskId: string, targetBranch: string): Promise<IPCResult<CreatePullRequestResult>> => {
      try {
        const { task, project } = findTaskAndProject(taskId);
        if (!task || !project) {
          return { success: false, error: 'Task not found' };
        ...
        const branch = execFileSync(gitPath, ['rev-parse', '--abbrev-ref', 'HEAD'], {
          cwd: worktreePath,
          encoding: 'utf-8'
        }).trim();
        ...
        let wasPushed = false;
        try {
          execFileSync(gitPath, ['rev-parse', '--abbrev-ref', `${branch}@{upstream}`], {
            cwd: worktreePath,
            encoding: 'utf-8',
            stdio: ['pipe', 'pipe', 'pipe']
          });
        } catch {
          // No upstream - push the branch
          wasPushed = true;
          try {
            execFileSync(gitPath, ['push', '-u', 'origin', branch], {
              cwd: worktreePath,
              encoding: 'utf-8'
            });
          } catch (pushError) {
            return {
              success: false,
              error: `Failed to push branch: ${pushError instanceof Error ? pushError.message : 'Unknown error'}`
            };
          }
        }
...
${task.description || 'Auto-generated from Auto Claude task.'}

---
Generated by Auto Claude`;

          const prOutput = execFileSync(ghPath, [
            'pr', 'create',
            '--base', targetBranch,
            '--head', branch,
            '--title', task.title,
          ...
          // Store PR info in implementation_plan.json (without changing task status)
          const specDir = path.join(project.path, project.autoBuildPath || '.auto-claude', 'specs', task.specId);
          ...
          const errorOutput = (prError as { stderr?: string })?.stderr || errorMsg;
          ...
          if (errorOutput.includes('no commits')) {
            return {
              success: false,
              error: 'No commits to create a pull request from'
            };
          }

          return {
            success: false,
            error: `Failed to create pull request: ${errorOutput}`
          };
        }
      } catch (error) {
